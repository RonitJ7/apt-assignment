...
def validate_data_integrity(df: pd.DataFrame, mapping: Dict[str, Any]) -> bool:
    ...
    if volume_col:
        ...
        if vol_mean <= max_price_mean:
            violations.append(f"Volume magnitude sus low: {vol_mean:.2f} vs max price {max_price_mean:.2f}")
...
def main():
    ...
    print("####### BR BR Mapping structure is valid")
    
    data_valid = validate_data_integrity(df, mapping)
    
    
    if structure_valid and data_valid:
        print("\n ###### BR BR All validations passed! Mapping seems correct.")
        sys.exit(0)
    else:
        print("\n ###### BR BR Validation failed. Please review your mapping.")
        sys.exit(1)
